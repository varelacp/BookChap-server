...
router.post('/signup', async (req, res, next) => {
  const { email, password, name, address, phoneNumber, role, profileImage } = req.body;
  ...
  try {
    ...
    res.json({ email: newUser.email, name: newUser.name, _id: newUser._id, address: newUser.address });
  } catch (error) {
    console.log('', error);
    next(error);
  }
